...
const create_keyid_pairs = () => {
    ...
    if(!jsonData?.ki_pairs)
        jsonData.ki_pairs = {}
            
    for(const dataObj in jsonData)
        for(const key in jsonData[dataObj])
            if(!jsonData.ki_pairs?.[key])
                jsonData.ki_pairs[key] = ''
}
...
const appendJson = async (new_data, reset = true) => {
    ...
    if(!file_exists){
        ...
        for(const dataObj in new_data){
            ...
            for(const key in new_data[dataObj])
                jsonData[dataObj][key] = new_data[dataObj][key]
        }
    }
    create_keyid_pairs();
