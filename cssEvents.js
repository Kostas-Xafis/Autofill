...
function OpenActions(e) {
	...
	const bkgroundDimTransition = [{ background: "rgb(0 0 0 / 0%)" }, { background: "rgb(0 0 0 / 50%)" }];
...
const changeIndDp = dragItem => {
	if (mouseOverCur_li !== -1) {
		const ind = mouseOverCur_li + 1,
			drop_li_p = $(`#indexes ul li:nth-child(${ind})`).removeClass("drop_to_index").addClass("show_trash").children();
...
// 	if (div.children[0].src === "") div.children[0].src = "./images/main/trash.png";

// 	const slide_kframes = [{ width: div_width <= 0.5 ? "0%" : `${div_width}px` }, { width: "100%" }];
...
// 	const slide_kframes = [{ width: "0%" }, { width: div_width >= 33 ? "100%" : `${div_width}px` }];
...
// 	anim.finished.then(() => {
// 		if (li.getAttribute("data-dir") === "fwd") return;
...
// 			undo_changes_stack.splice(
// 				undo_changes_stack.findIndex(undo => undo.ind == ind + 1),
// 				1
// 			);
// 			redo_changes_stack = [];
